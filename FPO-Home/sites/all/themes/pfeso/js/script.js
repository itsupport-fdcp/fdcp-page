...
(function ($, Drupal, window, document, undefined) {
  ...
  Drupal.behaviors.my_custom_behavior = {
    attach: function (context, settings) {
      ...
      $('.service-item-ajax-popup').magnificPopup({
        type: 'ajax'
      });
    }
  };
...
function magnfcPop() {
  jQuery('.view-dom-id-61a1415d569f888e1269547c720cc1fa').ready(function (){
    jQuery('.service-item-ajax-popup').magnificPopup({
      type: 'ajax'
    });
...
function equalizeHeightsRelative(selector) {
  var heights = new Array();
  jQuery(selector).each(function () {
    jQuery(this).css('min-height', '0');
    jQuery(this).css('max-height', 'none');
  ...
  var max = Math.max.apply(Math, heights);
  jQuery('.view-films .film-item').css('height', max + 'px');
}

function equalizeHeightsRelative2(selector) {
  var heights = new Array();
  jQuery(selector).each(function () {
    jQuery(this).css('min-height', '0');
    jQuery(this).css('max-height', 'none');
  ...
  var max = Math.max.apply(Math, heights);
  jQuery('.view-films .service-item').css('height', max + 'px');
}

function equalizeHeightsRelative3(selector) {
  var heights = new Array();
  jQuery(selector).each(function () {
    jQuery(this).css('min-height', '0');
    jQuery(this).css('max-height', 'none');
  ...
  var max = Math.max.apply(Math, heights);
  max = max + 20;
  jQuery('.twist-item').css('height', max + 'px');
}

// jQuery('.service-item-ajax-popup').on( "myCustomEvent", function() {
//   jQuery('.service-item-ajax-popup').magnificPopup({
//     type: 'ajax'
//   });
// });
...
jQuery(document).ready(function () {
  ...
  jQuery('.main-image-slider .field-items').slick({
    slidesToShow: 1,
    slidesToScroll: 1,
    arrows: true,
    dots: true,
    fade: false,
    asNavFor: '.thumb-image-slider .field-items',
  });


  jQuery('.thumb-image-slider .field-items').slick({
    slidesToShow: 3,
    slidesToScroll: 1,
    asNavFor: '.main-image-slider .field-items',
    arrows: false,
    dots: false,
    focusOnSelect: true
  });
  ...
  jQuery('.thumb-image-slider .field-items .slick-slide').removeClass('slick-active');

  // Set active class to first thumbnail slides
  jQuery('.thumb-image-slider .field-items .slick-slide').eq(0).addClass('slick-active');

  // On before slide change match active thumbnail to current slide
  jQuery('.main-image-slider .field-items').on('beforeChange', function (event, slick, currentSlide, nextSlide) {
    var mySlideNumber = nextSlide;
    jQuery('.thumb-image-slider .field-items .slick-slide').removeClass('slick-active');
    jQuery('.thumb-image-slider .field-items .slick-slide').eq(mySlideNumber).addClass('slick-active');
  });

  jQuery("#edit-combine").attr("placeholder", "Search");
  jQuery('.page-node-799 .form-item-combine').append('<span class="glyphicon glyphicon-search appended-search"></span>');
  jQuery('.page-node-801 .form-item-combine').append('<span class="glyphicon glyphicon-search appended-search"></span>');
  jQuery('.page-node-792 .form-item-combine').append('<span class="glyphicon glyphicon-search appended-search"></span>');
  ...
  "></span>*/
  jQuery('.film-item-ajax-popup').magnificPopup({
    type: 'ajax'
  });
  ...
  jQuery('.pager-item a').attr('onClick', 'magnfcPop()');

  if (jQuery('.meet-us-btn').length) {
    jQuery('.meet-us-btn').magnificPopup({
      type: 'inline'
    });
  } else {
    jQuery('#block-webform-client-block-828').hide();
  }

  jQuery('#block-block-40').addClass('mfp-hide');
  jQuery('.newsletter-popup-success').magnificPopup({
    type: 'inline'
  });

  if (!jQuery('.node-type-news .breadcrumb').length) {
    if (jQuery('.landing-banner .banner-content-inner').length) {
      // jQuery('.breadcrumb').html();
      jQuery('.landing-banner .banner-content-inner').append(jQuery('.breadcrumb').html());
    }
  }
  /*jQuery('.instagram-wrap').append('<a target="_blank" href="https://www.instagram.com/filmphilippines/"><i class="fa fa-instagram" aria-hidden="true"></i></a>');
  jQuery('.facebook-wrap').append('<a target="_blank" href="https://www.facebook.com/filmphilippinesnow/"><i class="fa fa-facebook-official" aria-hidden="true"></i></a>');
  jQuery('.twitter-wrap').append('<a target="_blank" href="https://twitter.com/filmphilippines"><i class="fa fa-twitter-square" aria-hidden="true"></i></a>');*/

  jQuery('.node-type-locations .breadcrumb ol li').eq(0).append("<li>Locations Database › </li>");
  if (jQuery('.locations-banner .banner-content-inner').length) {
    // jQuery('.breadcrumb').html();
    jQuery('.locations-banner .banner-content-inner').append(jQuery('.breadcrumb').html());
  }

  jQuery('#block-views-services-block ul, #block-views-services-block-6 ul').masonry({
    // options
  ...
  jQuery(function () {
    ...
    if (width >= '1200') {
      jQuery('.inview-animate').bind('inview', function (event, visible) {
        if (visible) {
  ...
  jQuery(window).load(function () {
    ...
    setTimeout(
      function () {
        jQuery('#eapps-instagram-feed-1 > a').hide();
      }, 2000);
  });


  // jQuery(window).load(function() {
  // 	var oldsize = jQuery('#map-container').width(),
  // 		newsize = oldsize*0.80,

  // 	jQuery('#map-container svg g').attr('transform', 'scale(' + (newsize / oldsize) + ')');
  // });
  ...
  jQuery('#facebook-modal .modal-close a').click(function (e) {
    jQuery(this).closest('#facebook-modal').toggleClass('show');
  });

  jQuery('.video-modal .close').click(function () {
    var parentid = jQuery(this).closest(".video-modal").attr("id");
    var parentiframe = "#" + parentid + " iframe";
    jQuery(parentiframe).attr("src", jQuery(parentiframe).attr("src"));
  });
  ...
  jQuery('.view-films .film-item .btn-turn').click(function (e) {
    jQuery(this).closest('.view-films .film-item').toggleClass('active');
  });

  jQuery(window).load(function () {
    equalizeHeightsRelative(jQuery('.view-films .film-item .back'));
  });
  jQuery(window).resize(function () {
    setTimeout(function () {
      equalizeHeightsRelative(jQuery('.view-films .film-item .back'));
  ...
  jQuery('.view-services .service-item .btn-turn').click(function (e) {
    jQuery(this).closest('.view-services .service-item').toggleClass('active');
  });
  jQuery(window).load(function () {
    equalizeHeightsRelative2(jQuery('.view-services .service-item .back'));
  });
  jQuery(window).resize(function () {
    setTimeout(function () {
      equalizeHeightsRelative2(jQuery('.view-services .service-item .back'));
  ...
  jQuery('.twist-item .btn-turn').click(function (e) {
    jQuery(this).closest('.twist-item').toggleClass('active');
  });
  jQuery(window).load(function () {
    equalizeHeightsRelative3(jQuery('.twist-item .back'));
  });
  jQuery(window).resize(function () {
    setTimeout(function () {
      equalizeHeightsRelative3(jQuery('.twist-item .back'));
  ...
  jQuery('#navigation-header .desk-toggle').click(function (e) {
    jQuery('#navigation-header .link-wrapper').toggleClass('menu-hide');
    jQuery('#navigation-header .link-wrapper').toggleClass('menu-show');
    jQuery('#navigation-header .desk-toggle').toggleClass('menu-hide');
    jQuery('#navigation-header .desk-toggle').toggleClass('menu-show');

  });

  jQuery(window).load(function () {
    equalizeHeights(jQuery('.footer-mob-img'));
  });
  jQuery(window).resize(function () {
    setTimeout(function () {
      equalizeHeights(jQuery('.footer-mob-img'));
  ...
  /*jQuery(window).load(function() {
    equalizeHeights(jQuery('.inquire-wrap .steps-wrap'));
  });
  jQuery(window).resize(function() {
    setTimeout(function() {
      equalizeHeights(jQuery('.inquire-wrap .steps-wrap'));
  ...
  jQuery(window).load(function () {
    equalizeHeights(jQuery('.loc-item .loc-info'));
  });
  jQuery(window).resize(function () {
    setTimeout(function () {
      equalizeHeights(jQuery('.loc-item .loc-info'));
  ...
  jQuery(window).load(function () {
    equalizeHeights(jQuery('.view-films .film-item .info-wrap .film-text'));
  });
  jQuery(window).resize(function () {
    setTimeout(function () {
      equalizeHeights(jQuery('.view-films .film-item .info-wrap .film-text'));
  ...
  jQuery(window).load(function () {
    equalizeHeights(jQuery('.view-services .service-item .info-wrap .service-text'));
  });
  jQuery(window).resize(function () {
    setTimeout(function () {
      equalizeHeights(jQuery('.view-services .service-item .info-wrap .service-text'));
  ...
  jQuery(window).load(function () {
    equalizeHeights(jQuery('.twist-item .info-wrap .twist-text'));
  });
  jQuery(window).resize(function () {
    setTimeout(function () {
      equalizeHeights(jQuery('.twist-item .info-wrap .twist-text'));
  ...
  jQuery(".front .location-list").owlCarousel({
  ...
  jQuery(".card-slider-inner .field-name-field-card-type-slider > .field-items").owlCarousel({
    // Most important owl features
    items: 1,
    itemsCustom: false,
    itemsDesktop: [1199, 1],
  ...
  // Change List to Select (Services Filter)
  // jQuery('#block-menu-menu-service-menu ul').each(function(){
  ...
  // d3.json(usMap, function(error, us) {
  ...
  // New Menu Navigation
  jQuery('.mobile-side-nav .toggle-sub').click(function () {
    var linkClicked = jQuery(this).attr('data-ref');
    jQuery(this).find(".fa").toggleClass('fa-chevron-down fa-chevron-up');
    jQuery('[data-menu="' + linkClicked + '"]').toggleClass('panel-active');
  });
  ...
  jQuery(".twt-share-button").click(function () {
    var url = window.location.href;
...
jQuery(window).load(function () {
  ...
  if (!newscat) return;
  jQuery('#edit-field-category-tid').val(newscat);
  jQuery('#edit-submit-news').click();
});